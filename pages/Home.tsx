...
const TECH_STACK = [
  { label: "Data Science", color: "text-red-600/25" },
  { label: "Full Stack", color: "text-green-600/25" },
  { label: "Machine Learning", color: "text-amber-600/25" },
  { label: "React TS", color: "text-red-700/25" },
  { label: "Python", color: "text-green-700/25" },
  { label: "Power BI", color: "text-amber-500/25" },
];
    ...
    <div className="min-h-screen flex flex-col pt-24 md:pt-32 overflow-hidden relative">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="mb-12 md:mb-16">
          <ScrambledText 
            className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter text-red-500 leading-tight select-none"
            radius={180}
            duration={0.7}
            speed={0.4}
            enableHover={true}
        ...
        <div className="max-w-6xl mx-auto">
          {/* Availability Badge */}
          <div className={`inline-flex items-center gap-3 mb-12 px-4 py-2.5 rounded-full border border-red-600/20 bg-gradient-to-r from-red-600/10 to-red-600/5 backdrop-blur-sm text-red-500 text-xs font-bold tracking-[0.2em] uppercase transition-all duration-700 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div className="relative mb-12">
            <div className={`transition-all duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
              ...
              <div className="relative">
                <ScrambledText 
                  className="text-[clamp(3rem,12vw,8rem)] font-black tracking-[-0.02em] text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-green-600 to-amber-500 leading-[0.85] uppercase select-none relative z-10"
                >
                  ARCHITECT
                </ScrambledText>
          ...
          <div className={`grid md:grid-cols-2 gap-8 md:gap-16 items-end mt-16 transition-all duration-1000 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div>
              <p className="text-xl md:text-2xl lg:text-3xl text-slate-300 font-medium leading-relaxed max-w-2xl">
                I transform <span className="text-white font-semibold relative before:absolute before:-inset-1 before:bg-red-600/20 before:rounded-lg before:-z-10">complex datasets</span> into{' '}
                <span className="text-white font-semibold relative before:absolute before:-inset-1 before:bg-green-600/20 before:rounded-lg before:-z-10">intuitive digital experiences</span>. 
            ...
            <div className="flex flex-col sm:flex-row gap-4 md:flex-col lg:flex-row md:justify-end">
              <button 
                ...
                className="group relative px-8 py-4 md:px-10 md:py-5 bg-gradient-to-r from-red-600 to-red-700 text-white font-bold rounded-full transition-all duration-300 hover:scale-[1.02] active:scale-95 shadow-xl shadow-red-600/25 hover:shadow-2xl hover:shadow-red-600/40 flex items-center justify-center gap-3"
              >
                <span>Explore Work</span>
                <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                <div className="absolute inset-0 rounded-full bg-gradient-to-r from-red-600 to-red-700 opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-300 -z-10"></div>
              </button>
              <button 
      ...
      <div className="mt-32 md:mt-48 relative">
        ...
        <div className="border-y border-white/5 py-8 md:py-12 rotate-[-0.5deg] bg-gradient-to-r from-red-600/5 via-transparent to-green-600/5 relative w-[110vw] left-[-5vw]">
                ...
                <div key={index} className="flex gap-8 md:gap-16 items-center">
                  {TECH_STACK.map((tech, techIndex) => (
                    <div key={`${index}-${techIndex}`} className="flex items-center gap-8 md:gap-16">
                      ...
                      <div className="w-2 h-2 rounded-full bg-red-600/10"></div>
                    </div>
                  ))}
                </div>
              ))}
            ...
            {MILESTONES.map((milestone, index) => (
              ...
              >
                <div className="relative z-10">
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-red-500 mb-4 flex items-center gap-2">
